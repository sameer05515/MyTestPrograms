...
//			int tokenTraversalIndex = index;
	...
	public static void main(String[] args) {
		...
		try {
			String chapterArrJsonFilePath = "D:\\Prem\\GIT-PROJ\\MyTestPrograms\\ShrimadBhagwatGeeta\\src\\main\\webapp\\data\\json\\chapter-verse-detail-with-mp3.json";
			...
			File[] mp3Arr=new File("D:\\Prem\\GIT-PROJ\\MyTestPrograms\\ShrimadBhagwatGeeta\\src\\main\\webapp\\audio").listFiles();
			int hhh=0;
			...
			for (JsonElement ec : chapterArr) {
				...
				for (JsonElement ev : (JsonArray) chapterObj.get("verses")) {
					...
					String shlokPath="audio/"+mp3Arr[hhh++].getName();
					verseObj.addProperty("shlokPath", shlokPath);
					
					System.out.printf("verseId %s\t\t shlokPath %s%n",verseId,shlokPath);
	...
	private static void AddVerseDetailAndWordMeaning() {
