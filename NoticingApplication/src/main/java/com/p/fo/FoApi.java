...
public class FoApi {
	...
	public void setTrdTm(String trdTm) {
		...
		Long jiffies = Long.valueOf(trdTm);

		this.trdTm = this.getTrdTime(jiffies);
	...
	public void setOrdTm(String ordTm) {
		...
		this.ordTm = "";
		/*
		 * Long jiff=Long.valueOf(ordTm); this.ordTm=this.getOrdTime(jiff);
		 */
	...
	public String toString() {
		return seqNo + "," + mkt + "," + trdNo + "," + trdTm + "," + tkn + "," + trdQty + "," + trdPrc + "," + bsFlg
				+ "," + ordNo + "," + brnCd + "," + usrId + "," + proCli + "," + cliActNo + "," + cpCd + "," + remarks
				+ "," + actTyp + "," + TCd + "," + ordTm + "," + booktype + "," + oppTmCd + "," + ctclId + "," + status
				+ "," + TmCd + "," + sym + "," + ser + "," + inst + "," + expDt + "," + strPrc + "," + optType + ","
	...
	public String getTrdTime(long dtValjiffies) {
		...
		try {
			...
			long sec = dtValjiffies / 65536;
			datetime = datetime.plusSeconds(sec);
			date = datetime.format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm:ss"));

		} catch (Exception e) {
			System.out.println("Exception while parsing getTrdTime ");
			Log.notice.error("Exception while parsing getTrdTime: " + dtValjiffies + " jiffes");
			date = "00/00/0000 00:00:00";
		}
		// System.out.println("TRADETIME: :=========>"+date);

		/*
		 * Log.notice.info("trade no.: "+trdNo + "| trdtimejiffies: "+ dtValjiffies
		 * +"| after getTrdTime: "+date );
		 */
	...
	public String getOrdTime(long orddtValjiffies) {
		...
		try {
			...
			Log.notice.error("Exception while parsing getOrdTime: " + orddtValjiffies + " jiffes");
			date = "00/00/0000 00:00:00";
		}
		// Log.notice.info("trade no.: "+trdNo + "| ordtimejiffies: "+ orddtValjiffies
		// +"| after getOrdTime: "+date );
		return date;
	}
	...
	public String getExpDate(long exprydatesecs) {
		...
		try {
			...
			Log.notice.error("Exception while parsing getExpDate: " + exprydatesecs);
			formattedDate = "00/00/0000";
		}

		/*
		 * Log.notice.info("trade no.: "+trdNo + "| expdateinsec: "+ exprydatesecs
		 * +"| after getExpDate: "+formattedDate );
		 */
